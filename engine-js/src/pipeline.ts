...
const generate = (typeof generateModule === "function" ? generateModule : (generateModule as unknown as { default: (ast: Node, opts?: object, code?: string) => { code: string } }).default) as unknown as (ast: Node, opts?: object, code?: string) => { code: string };
...
const PARSER_PLUGINS = ["typescript", "jsx", "decorators"] as const;

function* walkFiles(dir: string, base: string = dir): Generator<string> {
...
function parseFile(content: string, filename: string): Node {
...
export function run(
  ...
  for (const rel of walkFiles(inputRoot)) {
    ...
    let changed = false;
    const ctx: RunContext = { filePath: rel, recipeName: "" };
